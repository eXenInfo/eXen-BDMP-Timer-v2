...
import { getVolume, getStartSignalMs } from '../src/core/audio.js'
...
describe('Signaleinstellungen', () => {
  it('ohne gespeicherte Werte gelten 80 % und 600 ms', () => {
    expect(ladeSignale(speicher())).toEqual({ lautstaerke: 80, startMs: 600 })
    expect(ladeSignale(null)).toEqual({ lautstaerke: 80, startMs: 600 })
  })

  it('kaputter Eintrag fällt auf die Standardwerte zurück', () => {
    expect(ladeSignale(speicher({ [SCHLUESSEL]: '{kaputt' }))).toEqual({ lautstaerke: 80, startMs: 600 })
  })
  ...
  it('gespeicherte Werte überleben das Neuladen', () => {
    const s = speicher()
    sichereSignale(s, { lautstaerke: 40, startMs: 1000 })
    expect(ladeSignale(s)).toEqual({ lautstaerke: 40, startMs: 1000 })
  })

  it('wird auf die Signalausgabe übertragen', () => {
    wendeSignaleAn({ lautstaerke: 30, startMs: 800 })
    expect(getVolume()).toBe(30)
    expect(getStartSignalMs()).toBe(800)
    wendeSignaleAn({ lautstaerke: 80, startMs: 600 })
  })
})
